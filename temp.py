...
class Pi_manager:
    def __init__(self, id):
        ...
        self.client_socket.connect(('192.168.4.129', 9999))
        self.message = "Initial message from server."
        self.steps_per_turn = 1140
        self.overshoot = 0
        self.controller = PIDController(kp=0.00048, ki=0.000004, kd=0.0000000000001, setpoint=1140)
    def move_angle(self, steps_per_turn, angle, motor_number, direction = -1):
        ...
        if motor_number == 1:
            while True:
                print(self.count, self.count2, "in loop", self.controller.compute(self.count), steps)
                speed  = self.controller.compute(self.count)
                if speed < 0.3:
                   speed = 0.3
                self.kit.motor1.throttle = direction*speed
                if   -steps + self.count > 0:
                    self.kit.motor1.throttle = 0
                    self.overshoot = (self.count - steps)*1.1
    ...
    def receive_message(self):
        while "exit" not in self.message:
            ...
            if 'go' in self.message:
                self.kit.motor1.throttle = -0.45
                time.sleep(5)
                self.kit.motor1.throttle = 0
            else:
                try:
